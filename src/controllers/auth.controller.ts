...
export async function createUserSessionHandler(req: Request, res: Response) {
  ...
  // Create a session
  const session = (await createSession(user._id.toString(), req.get('user-agent') || '')) as any;
...
export async function invalidateUserSessionHandler(req: Request, res: Response) {
  ...
  return res.clearCookie('accessToken').clearCookie('refreshToken').send(msg(200, {}, 'Logout berhasil!'));
}
...
export async function refreshAccessToken(req: Request, res: Response) {
  ...
  await reIssueAccessToken(refreshToken)
    .then((accessToken) => {
      if (!accessToken) {
        return res.status(401).send(msg(401, { ...refreshToken }, 'Already logged out'));
      }
      return res.status(200).send(msg(200, { accessToken }, 'Session refreshed'));
    })
    .catch((err) => {
